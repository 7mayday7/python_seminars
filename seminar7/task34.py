...
poem = input('Введите стих: ').lower()
phrases = poem.split()
vowels = ['а', 'е', 'ё', 'и', 'о', 'у', 'ы', 'э', 'ю', 'я']

count_first_phrase = len(list(filter(lambda x: x in vowels, phrases[0])))

for phrase in phrases:
    count_vowel = len(list(filter(lambda x: x in vowels, phrase)))

    if count_vowel != count_first_phrase:
        print('Пам парам')
        break
